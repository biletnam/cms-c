...
class Banners extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
            ...
            array('name, link', 'match', 'pattern' => '/^[A-Za-z0-9\-_]+$/u', 'message' => 'Поле {attribute} должно содержать только латинские буквы, цифры и знак "-"!'),
			array('code, description', 'safe'),
			array('image', 'file', 'types' => 'gif, jpg, jpeg, png', 'allowEmpty' => true),
			array('id, name, title, description, image, link, code, content_type, views, clicks, notactive', 'safe', 'on'=>'search'),
		);
	}
